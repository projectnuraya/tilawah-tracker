...
Refer to `docs/` for full context: `docs/technical-requirements.md`, `docs/product-concept.md`, `docs/database-schema.md`, `docs/srs-docs.md`, `docs/ui-theme.md`, `docs/user-journeys.md`, `docs/api-docs.md`.
...
- **Route groups** under `src/app/`:
    ...
    - `api/v1/` — REST API (groups, participants, periods, progress, public)
...
- Input validation: Zod schemas in `src/components/lib/validators.ts`, run through `validateInput(schema, data)`, which throws/returns a `ValidationError`-shaped result. Add new schemas there rather than inlining `z.object` in route files.
...
- Rate limiting: `src/components/lib/rate-limit.ts` (in-memory, per-identifier token bucket — not multi-instance safe) plus `withRateLimit()` / header helpers in `src/components/lib/rate-limit-middleware.ts`. Pick the limiter matching the endpoint's cost (`auth`, `bulkParticipant`, `singleParticipant`, `progress`, `write`, `read`, `public`).
- Logging: `src/components/lib/logger.ts` (Pino; pretty-printed in dev, JSON in prod). Use structured fields (`logger.error({ err }, 'message')`), not string interpolation for errors.
