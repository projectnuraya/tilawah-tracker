	...
	return (
		<div className='relative' onClick={(e) => e.preventDefault()}>
			<select
				...
				disabled={isUpdating}
				className='appearance-none bg-transparent border border-border rounded-lg px-3 py-1.5 pr-8 text-sm font-medium text-foreground hover:bg-muted/50 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer'
			...
			{error && (
				<div className='absolute top-full mt-1 left-0 right-0 text-xs text-destructive bg-background border border-destructive rounded px-2 py-1 shadow-sm whitespace-nowrap'>
					{error}
				</div>
			)}
		</div>
