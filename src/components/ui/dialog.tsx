...
>(({ className, ...props }, ref) => (
	<DialogPrimitive.Overlay
		ref={ref}
		className={cn(
			'fixed inset-0 z-50 bg-black/60 backdrop-blur-xs data-[state=open]:animate-overlay-show data-[state=closed]:animate-overlay-hide',
			className,
		)}
		{...props}
	/>
		...
		<div className='pointer-events-none fixed inset-0 z-50 overflow-y-auto'>
			<div className='flex min-h-full items-center justify-center sm:p-6'>
				<DialogPrimitive.Content
					ref={ref}
					className={cn(
						'pointer-events-auto relative grid w-full max-w-lg gap-4 border bg-background p-6 shadow-xl data-[state=open]:animate-content-show data-[state=closed]:animate-content-hide sm:rounded-lg',
						className,
					)}
					{...props}>
					{children}
